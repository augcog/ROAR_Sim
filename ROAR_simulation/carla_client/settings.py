...
carla_client_folder_path = Path(os.getcwd()) / "ROAR_simulation" / "carla_client"
...
class CarlaSettings(BaseModel):
    ...
    carla_weather: CarlaWeather = Field(
        default=CarlaWeathers.SUNNY,
        title="Carla Weather",
        description="Weather Setting",
    )

    # carla vehicle setting
    carla_vehicle_blueprint_filter: str = Field(
    ...
    car_color: CarlaCarColor = Field(
        default=CarlaCarColors.RED,
        title="Vehicle color",
    ...
    waypoint_file_path: str = Field(
        default=(Path(os.getcwd()) / "data" / "easy_map_waypoints.txt").as_posix()
    )
    ...
    enable_autopilot: bool = Field(
        default=True, title="Enable Antopilot", description="Enable Antopilot"
    )
    spawn_point_id: int = Field(
        default=1, title="Spaning Location ID", description="Spanning Location ID"
    )

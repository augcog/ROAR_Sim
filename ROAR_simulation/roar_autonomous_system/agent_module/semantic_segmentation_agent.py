...
from ROAR_simulation.roar_autonomous_system.planning_module.local_planner.gpd_only_local_planner import (
...
class SemanticSegmentationAgent(Agent):
    def __init__(
        ...
        self.semantic_seg_detector = SemanticSegmentationDetector(
            vehicle=vehicle, camera=self.front_depth_camera
        )
        ...
        self.local_planner = SemanticSegmentationOnlyPlanner(
    ...
    def run_step(self, vehicle: Vehicle, sensors_data: SensorsData) -> VehicleControl:
        super(SemanticSegmentationAgent, self).run_step(
            vehicle=vehicle, sensors_data=sensors_data
        )
        self.semantic_seg_detector.run_step(
            vehicle=vehicle, new_data=sensors_data.front_depth.data
        )
        control = self.local_planner.run_step(vehicle=vehicle)
        self.visualizer.visualize_semantic_segmentation(
            semantic_segmetation=self.semantic_seg_detector.semantic_segmentation
        )
        return control

...
from ROAR_simulation.roar_autonomous_system.perception_module.ground_plane_detector import SemanticSegmentationDetector
...
from ROAR_simulation.roar_autonomous_system.planning_module.local_planner.gpd_only_local_planner import SemanticSegmentationOnlyPlanner
...
class SemanticSegmentationAgent(Agent):
    def __init__(self, vehicle: Vehicle, front_depth_camera: Camera, show_gpd_data=False):
        ...
        self.controller = VehiclePIDController(self.vehicle,
                                               ...
                                               target_speed=40)
        self.local_planner = SemanticSegmentationOnlyPlanner(vehicle=self.vehicle,
                                                             controller=self.controller,
                                                             gpd_detector=self.semantic_seg_detector,
    ...
    def run_step(self, vehicle: Vehicle, sensors_data: SensorsData) -> VehicleControl:
        super(SemanticSegmentationAgent, self).run_step(vehicle=vehicle, sensors_data=sensors_data)
        self.semantic_seg_detector.run_step(vehicle=vehicle, new_data=sensors_data.front_depth.data)
        control = self.local_planner.run_step(vehicle=vehicle)
        self.visualizer.visualize_semantic_segmentation(semantic_segmetation=

...
class OccupancyMap(BaseModel):
    data: np.ndarray = Field(
        ...,
        title="Occu Map Data",
        description="2D array containing probability of whether there is an obstacle. ",
    )
    ...
    @staticmethod
    def create_map(
        mission_plan: Union[List[Transform], None] = None,
        map_width: int = 1000,
        map_height: int = 1000,
        road_width: int = 15,
        ...
        if mission_plan is None:
            occupancy_grid = np.ones(
                (
                    map_width + map_additional_padding,
                    map_height + map_additional_padding,
                )
            )
            return OccupancyMap(
                data=occupancy_grid,
                map_additional_padding=map_additional_padding,
            ...
            occupancy_grid = np.ones(
                (
                    max(X_transformed) + map_additional_padding,
                    max(Y_transformed) + map_additional_padding,
                )
            )
            for x, y in zip(X_transformed, Y_transformed):
                occupancy_grid[
                    x - road_width // 2 : x + road_width // 2,
                    y - road_width // 2 : y + road_width // 2,
                ] = 0
            return OccupancyMap(
                data=occupancy_grid,
                road_width=road_width,
    ...
    def convert_to_map_coord(self, location: Location) -> Tuple[int, int, int]:
        return (
            np.ceil(location.x - self.x_min + self.map_additional_padding).astype(int),
    ...
    def visualize(
        self,
        center_coord: Union[None, Tuple[int, int]] = None,
        fov: int = 20,
        duration=1,
    ):
        """
        ...
        if center_coord is None:
            ...
            coord_values = vis_data[
                x - vehicle_width : x + vehicle_width,
                y - vehicle_width : y + vehicle_width,
            ]
            vis_data[
                x - vehicle_width : x + vehicle_width,
                y - vehicle_width : y + vehicle_width,
            ...
            y_max = (
                len(self.data[0]) - 1 if y + fov > len(self.data[0]) - 1 else y + fov
            )

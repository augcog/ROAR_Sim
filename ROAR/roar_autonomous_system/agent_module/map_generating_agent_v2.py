...
class MapGeneratingAgentV2(Agent):
    ...
    def run_step(self, sensors_data: SensorsData, vehicle: Vehicle) -> VehicleControl:
        super(MapGeneratingAgentV2, self).run_step(sensors_data, vehicle)
        self.ground_plane_detector.run_step()
        control = self.local_planner.run_step(self.vehicle)
        try:
            if self.ground_plane_detector.curr_segmentation is not None and \
                ...
                # find left and right width
                left_width = np.linalg.norm(points[0] - points[1])
                right_width = np.linalg.norm(points[1] - points[0])

                # put it into the log
                map_entry = MapEntry(
                    center=points[1].tolist(),
        ...
        if self.local_planner.is_done() and self.file_written is False:
            ...
            output_file_path:Path = Path(self.agent_settings.output_data_folder_path) / "easy_map_waypoints_v2.json"

...
class GPDOnlyLocalPlanner(LocalPlanner):
    ...
    def run_step(self):
        ...
        copy_depth = self.gpd_detector.curr_depth.data.copy()
        ...
        cv2.imshow("depth", copy_depth)
        print(self.vehicle.transform)
        next_way_point = None
        if is_front_clear:
            # generate front waypoint
            print("Going Straight")
            return Control(throttle=0.75, steering=0)

        elif is_right_clear:
            print("Turning Right")
            return Control(throttle=0.5, steering=0.2)
        elif is_left_clear:
            print("Turning Left")
            return Control(throttle=0.5, steering=-0.2)
        else:
